...
if (settings.syncVersionFromTag && gitInfo.tag) {
  ...
  if (parsedVersion) {
    ...
    if (manifestData.bp.manifest && manifestData.bp.path) {
      manifestData.bp.manifest.header.version = parsedVersion;
      fs.writeFileSync(
        manifestData.bp.path,
        JSON.stringify(manifestData.bp.manifest, null, 2),
        "utf8"
      );
    ...
    if (manifestData.rp.manifest && manifestData.rp.path) {
      manifestData.rp.manifest.header.version = parsedVersion;
      fs.writeFileSync(
        manifestData.rp.path,
        JSON.stringify(manifestData.rp.manifest, null, 2),
        "utf8"
      );
